...
const computeDatesBetween = (beginning, end, weekdays, timezone) => {
	...
	// todo: we're just generating dates, is there a faster way than Luxon?

	beginning = parseDate(beginning)
	end = parseDate(end)
	end = +DateTime.fromISO(end + 'T00:00', {zone: timezone})

	const dates = []
	let t = DateTime.fromISO(beginning + 'T00:00', {zone: timezone})
	for (let i = 0; t <= end; i++) {
		if (weekdays[weekdayIndexes[t.weekday]]) dates.push(t.toISODate())
		t = t.plus({days: 1})
	}
